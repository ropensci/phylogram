---
title: "The 'phylogram' package for evolutionary tree development in R"
...
building very large trees using fast alignment-free k-mer counting and 
sequence embedding techniques.
We hope that this will make R's powerful nested-list architecture more 
accessible to evolutionary biologists, and facilitate the analysis 
...
An alternative tree structure is the "dendrogram" object, produced using the 
`as.dendrogram` function in the **stats** package [@R2017].
...
which reads a character string or text files in the Newick
...
The current version of this package ignores inner-node labels; however, 
...
The `kdistance` function computes a fast, alignment-free distance matrix 
using k-mer counting, and returns an object of class "dist". 
...
k-mer (k-tuple) distance measure outlined by Edgar [-@edgar2004local].
...
### Sequence embedding
The `kdistance` operation has time and memory complexity $O(n^2)$ 
(where $n$ is the number of sequences), and thus can 
...
sized subset of 'seed' sequences (the default number of seeds is 
$log_2(n)^2$ as suggested by Blackshields et al. [-@blackshields2010sequence]).
...
embedded distance matrix with three randomly selected seed sequences, 
both using a k-mer size of 5.
...
### Compute the embedded distance matrix and print
set.seed(999)
...
In this example we will compare the fast and efficient top-down 
...
distance measure as featured in the examples in the **ape** package.
...
### (3) topdown tree with no distance matrix computation
set.seed(999)
# tree3 <- phylogram::topdown(woodmouse, seeds = seeds, weighted = TRUE)
tree3 <- phylogram::topdown(woodmouse, k = 5)
### sort nodes by size
...
the divisive (topdown) method builds trees orders of magnitude faster than 
