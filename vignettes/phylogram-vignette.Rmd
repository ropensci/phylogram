...
binary trees (Balding et al. 2003), and hence finding the true phylogeny is 
...
code-based workflow and the increased availability of powerful 
analytical tools developed within a suite of open-source packages. 
...
is the "phylo" object, a list whose primary element is an integer matrix 
...
way that standard lists are, which in addition to the availability of editing 
...
```read.dendrogram```.
This parser supports weighted edges, labels with special metacharacters 
(provided they are enclosed in single quotation marks), comments 
(enclosed in square brackets; ignored), multifuricating nodes, 
and both rooted and unrooted trees.
Inner-node labels are ignored by the parser in this version, with 
the addition of "label" attributes for non-leaf "dendrogram" nodes 
...
\end{equation}
$$
```kdistance``` produces a standard $N \times N$ distance matrix (where $N$ is the 
number of sequences), returning an object of class "dist". This operation has 
time and memory complexity $O(n^2)$ and hance can become computationally
infeasible for large sequence sets consisting of more than 10,000 or so sequences
(Blackshields et al. 2010).
The "phylogram" package also features the ```mbed``` function for "embedding" 
sequences in $t$ dimensional space (where $t \approx log_2(N)^2$) using the 
algorithm described in (Blackshields et al. 2010). 
This effectively linearizes the operation, making it possible to construct trees 
...
the "DNAbin" or "AAbin" raw-byte format (see the documentation in the ape package 
for more information on these classes). 
Sequences supplied in the "character" format are supported; however ambiguity 
codes may not be recognized or treated appropritely. 
...
This will not happen if amino acid sequences are supplied in character format 
rather than as an "AAbin" object, in which case the k-mer length should be reduced 
...
the function ```topdown``` that builds a tree by recursively splitting the 
...
..not satisfy a taxonomist.. sequence weighting and giude trees..

and the export function
```write.dendrogram``` provides the facility to
