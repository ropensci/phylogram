...
editing, visualizing and publishing evolutionary trees in a suite of 
comprehensive R packages. 
Here we introduce **phylogram**, an R package
that interfaces traditional matrix-based evolutionary tree formats
with a data structure in which trees are stored 
as deeply-nested lists.
In contrast to the widely used "phylo" object type, 
in which a tree is represented as a matrix of edges, 
...
includes functions for the rapid divisive construction of trees from very large
...
These features will be useful as we continue to develop new 
...
binary trees [@balding2008handbook], and hence reconstructing the true 
...
become available. This dynamic nature necessitates computational tools 
that facilitate fine-scale tree manipulation, offer multiple 
visualization options, and are readily scalable for large data-sets.
...
These packages employ a tree representation called the "phylo" object, 
...
As opposed to a matrix of edges, a dendrogram object is a heirarchical list 
...
Each node of a dendrogram object has the following attributes: 
...
* "midpoint" the horizontal distance of the node from the left-most member 
   of the subtree, where the horizontal distance between the leftmost and 
   rightmost leaves of the subtree is 1
...
can be attached to inner nodes. 
This can be a powerful feature when performing recursive tree operations 
in either a top-down (divisive) or bottom-up (agglomerative) fashion, 
as described in further detail below. 

#### Example 1: build a dendrogram object _de novo_
...
The syntax is similar when reading from text files, 
...
alignment-free k-mer or k-tuple distance measure outlined by Edgar [-@edgar2004local].
...
However, an alternative option detailed in Yang & Zhang [-@yang2008performance] 
can be invoked by
...
quickly become computationally infeasible for sequence sets with more than 
~ 10,000 sequences.
...
using the algorithm described in Blackshields et al. [-@blackshields2010sequence]. 
This speeds up the operation dramatically, making it possible to reconstruct trees 
from over a million ~ 200 nt DNA sequences in under an hour *via* recursive 
...
```ape::dist.dna```. First, we load the woodmouse dataset and view the first few 
columns as follows:
...
Notice that this is a semi-global (a.k.a "glocal") alignment featuring some 
...
the tally for "ACAGT" and 0.5 to that of "ACGGT".  
...
Hence if 'seeds' is set to NULL (the default) the the ```mbed``` 
function selects the subset by clustering the sequence set into 
$t$ groups using the k-means algorithm, 
...
### Assembling trees
There are several tools available for evolutionary tree 
construction from a given distance matrix, 
...
However, these are bottom-up (agglomerative) algorithms that 
rely on a $n \times n$ distance matrix and are therefore 
unsuitable for embedded sequences.
The **phylogram** package features the function ```topdown``` 
that builds a tree by recursively splitting the 
(embedded) sequence set using k-means clustering ($k = 2$). 
...
distance measure
(as used in the **ape** package examples).
...
### Tree vizualization 
Publication-quality trees can be generated from dendrogram objects relatively easily 
...
The phylogram package therefore does not currently offer any additional plotting 
functions, instead focusing on tree generation and manipulation (see next section), 
particularly for larger sequence datasets.
